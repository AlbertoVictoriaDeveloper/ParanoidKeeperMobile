...
$("#send").click(function(){
    ...
    if (!res && res == "") {
        ...
        $.ajax({async: true,
            type: "POST",
            dataType: "JSON",
            url: "http://192.168.1.66/paranoidkeeper/login/dataregister",
            data: data,
            success: function (data) {
                if (data.response == "OK") {
                    ...
                    $(".inline").colorbox({inline:true, width:"100%",height:"45%",open:true});
    ...
    if (!res  && res == "") {
       ...
       $("#alert").fadeOut(6000); 
        ...
        $.ajax({async: true,
            type: "POST",
            dataType: "JSON",
            url: "http://192.168.1.66/paranoidkeeper/login/registerUser",
            data: data,
            success: function (data) {
                if (data.response == "OK") {
                    $("#alert").attr("style", "display:none");
...
$("#login").click(function(){
    ...
    if (!mail && !pass) {
        ...
        $.ajax({async: true,
            type: "POST",
            dataType: "JSON",
            url: "http://192.168.1.66/paranoidkeeper/login/accessmobile",
            data: data,
            success: function (data) {
                if (data.response == "ok") {
					localStorage.setItem("token",data.info['key']);
					//localStorage.setItem("template","seepass.html");
